<%-- 
    Document   : inicio
    Created on : 14/05/2016, 03:20:57
    Author     : Wensttay
--%>

<!DOCTYPE html>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<article class="container-base  exibindo" id="inicio">
    <div class="container text-center">
        <div class="row">

            <div class="evento-box col-md-12">    
                <h1 class="evento-nome low-transparent">III ECMAT</h1>
                <h3 class="low-transparent">Terceiro Encontro Cajazeirense de Matemática</h3>
            </div>

            <div class="col-md-12">
                <div class="evento-info-box low-transparent col-md-4 col-sm-4">
                    <a href="#programacao">
                        <h3 class="low-transparent">Dias 24 a 26 de Agosto</h3>
                        <img class="evento-info-img" src="img/other/calendario.png"/>
                    </a>
                </div>
                <div class="evento-info-box low-transparent col-md-4 col-sm-4">
                    <a href="#contato">
                        <h3 class="low-transparent">IFPB - Campus Cajazeiras</h3>
                        <img class="evento-info-img" src="img/other/posicao.png"/>
                    </a>
                </div>
                <div class="evento-info-box low-transparent col-md-4 col-sm-4 disabled">
                    <a href="#construcao">
                        <h3 class="low-transparent">Inscrições a partir de 15 de Junho</h3>
                        <img class="evento-info-img" src="img/other/inscricao.png"/>
                    </a>
                </div>
            </div>
        </div>
    </div>
</article>
