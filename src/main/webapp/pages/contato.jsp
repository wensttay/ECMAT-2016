<%-- 
    Document   : sobre
    Created on : 14/05/2016, 03:21:22
    Author     : Victor Hugo
--%>

<!DOCTYPE html>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<article class="container-base invisivel " id="contato">
    <div class="container contato">
        <div class="row">
            <div class="col-md-1"></div>
            <div class="col-md-10 text-justify"> 
                <p class="text-center article-title col-sm-12">Contato</p>
                <div class="col-md-6 div_mapa"> 
                    <iframe class="mapa off-load" src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3961.0079620122965!2d-38.54710438567306!3d-6.889648769332253!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x7a473a32378c727%3A0x4e45e65b60da5295!2sIFPB+-+Campus+Cajazeiras!5e0!3m2!1spt-BR!2sbr!4v1463258403761" frameborder="0" style="border:0" allowfullscreen></iframe>
                </div>  
                <div class="col-md-6 container-dados-contato"> 
                    <div class="dados-contato">
                        <p><i class="material-icons padding-medium">&#xE55F;</i>Rua José Antônio da Silva, 300, B. Jardim Oásis</p>
                        <p><i class="material-icons padding-medium">&#xE0BE;</i>ecmat2016@gmail.com</p>
                        <p><i class="material-icons padding-medium">&#xE0B0;</i>(83) 3532-4100 Ramal: 4186</p>
                        <p><i class="material-icons padding-medium">&#xE32C;</i>(83) 99640-1322</p>
                    </div>
                </div>
            </div>
            <div class="col-md-1"></div>
        </div>
    </div>
</article>