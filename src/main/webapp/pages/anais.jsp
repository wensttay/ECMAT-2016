<%-- 
    Document   : anais
    Created on : 29/05/2016, 16:48:23
    Author     : wensttay
--%>

<!DOCTYPE html>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<article class="container-base invisivel " id="anais">
    <div class="container">
        <div class="row">
            <div class="col-md-12 sumario">
                <ul>
                    <li>
                        <a href="#sobre">Apresentação do III Ecmat 2016</a>
                    </li>
                    <li>
                        <a href="#organizacao">Organização e Corpo Editorial</a>
                    </li>   
                    <li>
                        <a href="#submissoes">Submissões e Critérios de Avaliação</a>
                    </li>
                </ul>
            </div>

            <p class="text-center article-title col-sm-12">Corpo Editorial</p>

            <div class="col-md-12">
                <ul>
                    <li class="titulo-comissoes-anais">
                        Comissão Organizadora
                        <table class="tabela-comissoes">
                            <tr>
                                <td>Prof. Msc.</td>
                                <td> <strong>José Marcos da Silva</strong>. </td>
                                <td>E-mail: <span class="email-comissoes-anais">josemarcos08@yahoo.com.br</span></td>
                            </tr>
                            <tr>
                                <td>Prof. Esp.</td>
                                <td><strong>Balboíno Sonildo da Nóbrega.</strong></td>
                                <td>E-mail: <span class="email-comissoes-anais">baldoino.nobrega@ifpb.edu.br</span></td>
                            </tr>
                            <tr>
                                <td>Prof. Esp.</td>
                                <td><strong>José Nunes Aquino.</strong></td>
                                <td>E-mail: <span class="email-comissoes-anais">nuneshaglaene@gmail.com</span></td>
                            </tr>
                            <tr>
                                <td>Prof. Esp.</td>
                                <td><strong>Francisco Aureliano Vidal.</strong></td>
                                <td>E-mail: <span class="email-comissoes-anais">aurelianovidal@hotmail.com</span></td>
                            </tr>
                            <tr>
                                <td>Prof. Esp.</td>
                                <td><strong>Geraldo Herbetet de Lacerda.</strong></td>
                                <td>E-mail: <span class="email-comissoes-anais">geraldo.lacerda@ifpb.edu.br</span></td>
                            </tr>
                        </table>
                        <!--                        <ul>
                                                    <li>Prof. Msc. <strong>José Marcos da Silva</strong>. E-mail: josemarcos08@yahoo.com.br</li>
                                                    <li>Prof. Esp. <strong>Balboíno Sonildo da Nóbrega</strong>. E-mail: baldoino.nobrega@ifpb.edu.br</li>
                                                    <li>Prof. Esp. <strong>José Nunes Aquino</strong>. E-mail: nuneshaglaene@gmail.com</li>
                                                    <li>Prof. Msc. <strong>Francisco Aureliano Vidal</strong>. E-mail: aurelianovidal@hotmail.com</li>
                                                    <li>Prof. Msc. <strong>Geraldo Herbetet de Lacerda</strong>. E-mail: geraldo.lacerda@ifpb.edu.br</li>
                                                </ul>-->
                    </li>
                    <li class="titulo-comissoes-anais"> Comitê Científico
                        <table class="tabela-comissoes">
                            <tr>
                                <td>Prof. Msc.</td>
                                <td> <strong>Nádia Pinheiro Nóbrega.</strong></td>
                                <td>E-mail: <span class="email-comissoes-anais">nadia.nobrega@ifpb.edu.br</span></td>
                            </tr>
                            <tr>
                                <td>Prof. Esp.</td>
                                <td><strong>Patrício Luiz de Andrade.</strong></td>
                                <td>E-mail: <span class="email-comissoes-anais">patricio.luiz.andrade@gmail.com</span></td>
                            </tr>
                            <tr>
                                <td>Prof. Esp.</td>
                                <td><strong>José Marcos da Silva.</strong></td>
                                <td>E-mail: <span class="email-comissoes-anais">josemarcos08@yahoo.com.br</span></td>
                            </tr>
                            <tr>
                                <td>Prof. Esp.</td>
                                <td><strong>Leonardo Ferreira Soares.</strong></td>
                                <td>E-mail: <span class="email-comissoes-anais">leonardo.ferreira@ifpb.edu.br</span></td>
                            </tr>
                            <tr>
                                <td>Prof. Esp.</td>
                                <td><strong>Thiago Andrade Fernandes.</strong></td>
                                <td>E-mail: <span class="email-comissoes-anais">thiago_taf@hotmail.com</span></td>
                            </tr>
                            <tr>
                                <td>Prof. Esp.</td>
                                <td><strong>Francisco Aureliano Vidal.</strong></td>
                                <td>E-mail: <span class="email-comissoes-anais">aurelianovidal@hotmail.com</span></td>
                            </tr>
                            <tr>
                                <td>Prof. Esp.</td>
                                <td><strong>Antônia Edivaneide de Sousa Gonzaga.</strong></td>
                                <td>E-mail: <span class="email-comissoes-anais">edivaneidesousa2012@gmail.com</span></td>
                            </tr>
                            <tr>
                                <td>Prof. Esp.</td>
                                <td><strong>Reginaldo Amaral Cordeiro Júnior.</strong></td>
                                <td>E-mail: <span class="email-comissoes-anais">reginaldoajunior@gmail.com</span></td>
                            </tr>
                            <tr>
                                <td>Prof. Esp.</td>
                                <td><strong>Valdecir Teófilo Moreno.</strong></td>
                                <td>E-mail: <span class="email-comissoes-anais">valdecir.moreno@ifpb.edu.br</span></td>
                            </tr>
                        </table>
                        <!--                        <ul>
                                                    <li>Prof. Msc. <strong>Nádia Pinheiro Nóbrega</strong>. E-mail: nadia.nobrega@ifpb.edu.br</li>
                                                    <li>Prof. Msc. <strong>Patrício Luiz de Andrade</strong>. E-mail: patricio.luiz.andrade@gmail.com</li>
                                                    <li>Prof. Msc. <strong>José Marcos da Silva</strong>. E-mail: josemarcos08@yahoo.com.br</li>
                                                    <li>Prof. Msc. <strong>Leonardo Ferreira Soares</strong>. E-mail: leonardo.ferreira@ifpb.edu.br</li>
                                                    <li>Prof. Msc. <strong>Thiago Andrade Fernandes</strong>. E-mail: thiago_taf@hotmail.com</li>
                                                    <li>Prof. Msc. <strong>Francisco Aureliano Vidal</strong>. E-mail: aurelianovidal@hotmail.com</li>
                                                    <li>Prof. Msc. <strong>Antônia Edivaneide de Sousa Gonzaga</strong>. E-mail: edivaneidesousa2012@gmail.com</li>
                                                    <li>Prof. Msc. <strong>Reginaldo Amaral Cordeiro Júnior</strong>. E-mail: reginaldoajunior@gmail.com</li>
                                                    <li>Prof. Msc. <strong>Valdecir Teófilo Moreno</strong>. E-mail: valdecir.moreno@ifpb.edu.br</li>
                                                </ul>-->
                    </li>
                    <li class="titulo-comissoes-anais">
                        Equipe Técnica/Operacional
                        <table class="tabela-comissoes">
                            <tr>
                                <td>Direção Geral do Campus:</td>
                                <td> <strong>Lucrécia Teresa Gonçalves Petrucci</strong>. </td>
                            </tr>
                            <tr>
                                <td>Diretoria de Ensino:</td>
                                <td><strong>Gastão Coelho de Aquino Filho.</strong></td>
                            </tr>
                            <tr>
                                <td>Coordenação de Comunicação:</td>
                                <td><strong>Lidiane Maria da Silva.</strong></td>
                            </tr>
                            <tr>
                                <td>Coordenação do Curso de Licenciatura em Matemática:</td>
                                <td><strong>Prof. Baldoino Sonildo da Nóbrega.</strong></td>
                            </tr>
                        </table>
                        <!--                        <ul>
                                                    <li>Direção Geral do Campus: <strong>Lucrécia Teresa Gonçalves Petrucci</strong>
                                                    </li>
                                                    <li>
                                                        Diretoria de Ensino: <strong>Gastão Coelho de Aquino Filho</strong>  
                                                    </li>
                                                    <li>
                                                        Coordenação de Comunicação: <strong>Lidiane Maria da Silva</strong>
                        
                                                    </li>
                                                    <li>
                                                        Coordenação do Curso de Licenciatura em Matemática: <strong>Prof. Baldoino Sonildo da Nóbrega</strong>
                                                    </li>
                                                </ul>-->
                    </li>
                </ul>

            </div>


            <div class="col-md-12 container-edicoes">
                <p class="text-center article-title col-sm-12">Edições</p>
                <div class="col-lg-6 col-md-6 col-sm-12 program-conteiner">
                    <div class="program-day-box"> <!-- Open Program-Day-Box -->
                        <a class="text-center numero-edicao" href="">
                            Segunda Edição
                        </a>
                        <div class="col-md-12 col-sm-12 img-edicao-box">
                            <div class="container-ano-edicao text-center">
                                <!--<p class="ano-edicao">2016</p>-->
                                <a href="http://www.google.com">
                                    <img src="img/logo/logo-ecmat2016-420x420.png">
                                </a>
                            </div>
                        </div>
                        <!--<p style="font-size: 60px; text-align: center; top: ">Dia</p>-->
                        <!-- Clear to full box Height -->
                        <span style="clear: both; content: ''; display: block; margin: 0;"></span>

                        <!--                        <div class="hour-box-list container-documentos-anais" hidden>  Open List box with hours 
                                                    <div class="documentos-anais">
                                                        PDF'S
                                                    </div>
                                                </div>  Close List box with hours -->
                    </div> 
                </div>
                <div class="col-lg-6 col-md-6 col-sm-12 program-conteiner">
                    <div class="program-day-box program-conteiner"> <!-- Open Program-Day-Box -->
                        <a class="text-center numero-edicao" href="">
                            Terceira Edição
                        </a>
                        <div class="col-md-12 col-sm-12 img-edicao-box">
                            <div class="container-ano-edicao text-center">
                                <!--<p class="ano-edicao">2016</p>-->
                                <a href="http://www.google.com">
                                    <img src="img/logo/logo-ecmat2015-420x420.png">
                                </a>
                            </div>
                        </div>
                        <!--<p style="font-size: 60px; text-align: center; top: ">Dia</p>-->
                        <!-- Clear to full box Height -->
                        <span style="clear: both; content: ''; display: block; margin: 0;"></span>

                        <!--                        <div class="hour-box-list container-documentos-anais" hidden>  Open List box with hours 
                                                    <div class="documentos-anais">
                                                        PDF'S
                                                    </div>
                                                </div>  Close List box with hours -->
                    </div> 
                </div>

            </div>
        </div>
    </div>
</article>
